...
__all__ = [
    ...
    # Backwards-compatible API (arbitrary-VDF default)
    "spectral_density_arbdist",
    "spectral_density_maxwellian",
    "fast_spectral_density_arbdist",
    "fast_spectral_density_maxwellian",
    "scattered_power_model_arbdist",
    "scattered_power_model_maxwellian",

    # Autodiff aliases (explicit)
    "spectral_density_arbdist_autodiff",
    "fast_spectral_density_arbdist_autodiff",
...
# extend __all__ in this unified module (defined above) with the PlasmaPy API
try:
    __all__ += ["spectral_density_lite", "spectral_density", "spectral_density_model"]
...
# =============================================================================
# Add PlasmaPy Thomson API (from thomson_plasmampy.py)
# =============================================================================
# These functions are added alongside the arbitrary-VDF + autodiff forward models
# above, without modifying their behavior.
#
# Provides:
...
#
# If optional dependencies are missing, importing this module will still work,
# but calling these functions will raise a helpful ImportError.
# =============================================================================
...
try:
    ...
    import astropy.units as u
    import numpy as np
    # lmfit is optional; required only for spectral_density_model
    try:
    ...
    def spectral_density_model(  # noqa: C901, PLR0912, PLR0915
        ...
        if Model is None:
            raise ImportError(
                "lmfit is required for spectral_density_model, but it is not installed. "
                "Install it with `pip install lmfit` to use this functionality."
            )
        return Model(
...
except Exception as _plasmapy_thomson_err:  # pragma: no cover
    def spectral_density_lite(*args, **kwargs):
        raise ImportError(
            "PlasmaPy-style spectral_density_lite could not be loaded. "
            f"Original error: {_plasmapy_thomson_err}"
        )

    def spectral_density(*args, **kwargs):
        raise ImportError(
            "PlasmaPy-style spectral_density could not be loaded. "
            f"Original error: {_plasmapy_thomson_err}"
        )

    def spectral_density_model(*args, **kwargs):
        raise ImportError(
            "PlasmaPy-style spectral_density_model could not be loaded. "
            f"Original error: {_plasmapy_thomson_err}"
        )
