...
# =============================================================================
# PATCH LAYER (minimal-change integration)
# =============================================================================
# This module historically merged multiple Thomson implementations.
# The merged file (thomson-5.py) previously exposed a PlasmaPy-style API under
# *_experimental names. You requested:
#   - Keep the existing merged code above unchanged
#   - Expose Thomson-6 as the STANDARD PlasmaPy API (spectral_density, etc.)
#   - Expose thomson2.py functionality as *_experimental (including spectral_power*)
#
# To minimize edits and preserve exact behavior, we embed the reference sources
# for thomson-6.py and thomson2.py and execute them into isolated namespaces,
# then re-export the desired callables here.

_THOMSON6_SOURCE = r"""__all__ = [
    "spectral_density",
...
def _exec_embedded_source(_src: str, _tag: str) -> dict:
    _ns: dict = {"__name__": f"plasmapy.diagnostics.thomson._embedded_{_tag}"}
    exec(_src, _ns, _ns)
    return _ns

# ---- Standard PlasmaPy Thomson API (from thomson-6.py) ----
_th6 = _exec_embedded_source(_THOMSON6_SOURCE, "thomson6")

# Export standard names exactly as in thomson-6.py
spectral_density_lite = _th6.get("spectral_density_lite")
spectral_density = _th6.get("spectral_density")
spectral_density_model = _th6.get("spectral_density_model")

# ---- Experimental API (from thomson2.py) ----
_th2 = _exec_embedded_source(_THOMSON2_SOURCE, "thomson2")

spectral_density_lite_experimental = _th2.get("spectral_density_lite")
spectral_density_experimental = _th2.get("spectral_density")
spectral_density_model_experimental = _th2.get("spectral_density_model")

# Additional thomson2 features (spectral power)
spectral_power_lite_experimental = _th2.get("spectral_power_lite")
spectral_power_lite2_experimental = _th2.get("spectral_power_lite2")
spectral_power_experimental = _th2.get("spectral_power")

# Re-bind forward-model alias for the experimental API to ensure it points at the
# thomson2-backed implementation (overriding any earlier binding).
experimental_forwardmodel = spectral_density_experimental

# Update __all__ (keep existing entries; only extend with the standard & extra experimental)
try:
    __all__.extend([
...
# Clean up embedded namespace references (optional; keeps module namespace tidy)
del _th6, _th2
